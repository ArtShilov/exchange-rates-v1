...
router.get("/manualLoader/:dateFrom/:dateTo", async function (
  req,
  res,
  next
) {
...
router.get("/course-valute-on-date/:date", async function (req, res, next) {
  ...
  const jsonOfDB = await readRecordsFromTableOnDay(date);

  if (jsonOfDB.ValCurs.Valute.length === 0) {
...
router.get(
  "/course-valute-on-period/:dateFrom/:dateTo/:valID",
  async function (req, res, next) {
    ...
    const jsonOfDB = await readRecordsFromTableOnPeriod(
      dateFrom,
      dateTo,
      valID
    );
